...
- Каталог возможностей для пользователей: `docs/capabilities/` — по странице на каждый из
  16 инструментов (что делает, что меняет, что нужно для вызова) плюс индекс; контракт
  оформления — `docs/a1-mcp-capability-documentation-contract.md`.

### Изменено

- Сервер больше не завершается при отсутствии токенов: он стартует, отвечает на `initialize`
  и `tools/list`, а в `instructions` предупреждает модель, какого токена не хватает и что
  оператор должен задать переменные окружения и перезапустить сервер. Прежние тексты
  стартовых ошибок («YANDEX_DELIVERY_TOKEN is required…», «Express-contour token is
  missing…», «Platform-contour token is missing…») сохранены дословно и возвращаются теперь
  при вызове инструмента (`CredentialsError`, ответ с `isError`). Проверка контурная и
  выполняется до построения запроса, ретраев и обращения к сети, поэтому наполовину
  настроенный сервер продолжает обслуживать контур, у которого токен есть. Раньше процесс
  умирал до MCP-хендшейка, и пользователь видел только «красный крест» без причины.
- Телеметрия: у выжившего неконфигурированного старта своё событие `unconfigured_start`
  с прежними кодами причин (`missing_token` / `missing_express_token` /
  `missing_platform_token`; при отсутствии обоих токенов — `missing_token`, как в старом
  порядке проверок); `server_start` теперь означает старт рабочей установки.
  `startup_failed` остаётся для невалидных значений конфигурации (таких проверок сейчас
  нет) и отправляется fire-and-forget — процесс больше не ждёт пинг перед выходом.
