 ...
 * still means "config unusable at load time" (malformed values; no such checks
 * exist today); it is kept unchanged so the historical funnel stays comparable.
 * `unconfigured_start` is its live counterpart — the server now survives
 * missing tokens and completes the handshake, so this is the first event that
 * can carry the client's name for an install that has no token in yet.
 */
